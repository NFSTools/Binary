...
namespace Binary.Support
{
	public partial class MostWanted : Form
	{
		...
		private void DataSet_SaveFile_Click(object sender, EventArgs e)
		{
			var watch = new System.Diagnostics.Stopwatch();
			watch.Start();
			Process.SaveData(dbMW, Properties.Settings.Default.EnableCompression);
			watch.Stop();
		...
		{
			this.MostWanted_FormClosed(this, null);
			this.Close();
		}

		private void MostWanted_FormClosing(object sender, FormClosingEventArgs e)
		{
		...
		private void MostWanted_FormClosed(object sender, FormClosedEventArgs e)
		{
			this.dbMW = null;
		...
		{
			var launch = new System.Diagnostics.ProcessStartInfo("speed.exe")
			{
		...
		{
			MessageBox.Show(this.dbMW.GetDatabaseInfo(), "Database Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
		}
		...
		private void EndscriptToolStripMenuItemI_Click(object sender, EventArgs e)
		{
			...
			{
				if (Core.ProcessEndscript(OpenEndscriptDialog.FileName, dbMW, out var label, out var text))
				{
